...
def solution(instructions)
  ...
  instructions.split(/\n/).each do |instruction|
    match = instruction.match(/^(\w+)\s(\w+)\s(-?\d+)\sif\s((\w+)\s[^\s]+\s-?\d+)/)
    register = match[1]
    operation = match[2]
    value = match[3]
    condition = match[4]
    condition_register = match[5]

    condition = condition.gsub(condition_register, "registers['#{condition_register}']")

    registers[register] ||= 0
    registers[condition_register] ||= 0

    condition_check = eval(condition)
    if condition_check
      if operation == 'inc'
...
EOS
    ...
    assert_equal 10, solution(instructions)
  end
...
EOS
    ...
    assert_equal 5035, solution(instructions)
  end
end