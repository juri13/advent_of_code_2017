...
class Program
  ...
  def initialize(name, weight, children = [])
    ...
    @real_weight = nil
    @children = children
    @children_weights = nil
    @all_programs = nil
  end

  def is_balanced?(all_programs)
    @all_programs ||= all_programs
    children_weights.uniq.count == 1
  end

  def all_children_balanced?(all_programs)
    @all_programs ||= all_programs
    @children.all?{ |child_name| @all_programs[child_name].is_balanced?(@all_programs) }
  end

  def children_weights
    @children_weights ||= @children.inject({}) do |hash, child_name|
      child_program = @all_programs[child_name]
      hash.merge({ child_name => child_program.compute_weight(@all_programs) })
    end
  end

  def compute_weight(all_programs)
    @all_programs ||= all_programs
    @real_weight ||= @weight + children_weights.values.inject(0, :+)
  end
end
...
def solution(input_string)
  ...
  input_string.split(/\n/).each do |row|
    match = row.match(/^(\w+)\s\((\d+)\)(\s->\s?([^\n]*))?/)
    name = match[1]
    weight = match[2].to_i
    children = match[4] ? match[4].split(', ') : []

    all_programs[name] = Program.new(name, weight, children)
    children.each { |child| programs_with_parents << child }
  end

  root_program_name = (all_programs.keys - programs_with_parents)[0]

  current_program = all_programs[root_program_name]

  until current_program.is_balanced?(all_programs) || current_program.all_children_balanced?(all_programs)
    weight_histogram = current_program.children_weights.values.group_by{ |v| v }
    supposed_children_weight = weight_histogram.detect{ |_, v| v.size > 1}[0]
    invalid_children_weight = weight_histogram.detect{ |_, v| v.size == 1}[0] rescue break
    next_program_name = current_program.children_weights.detect{ |_, v| v == invalid_children_weight }[0]

    current_program = all_programs[next_program_name]

    supposed_weight = current_program.weight - (invalid_children_weight - supposed_children_weight)
  end
...
EOS
    ...
    assert_equal 60, solution(input_string)
  end
...
EOS
    ...
    assert_equal 1219, solution(input_string)
  end
end